...
public class UserManageServiceBean implements UserManageService {
    ...
    public ResultMessage emailConfirmation(int userId, String emailAddress) {
        ...
        if (userDao.findByEmail(emailAddress).getId() == -1) {
    ...
    public MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail) throws Exception {
        ...
        message.setContent("<html><head></head><body><h1>尊敬的用户您好，这是一封来自Dingo的邮箱验证邮件，请点击链接进行邮箱验证</h1><h3><a href='http://localhost:8080/user/emailReConfirmation'></h3><h1>如果不是您发起的验证请求，请忽略本邮件</h1></body></html>", "text/html;charset=UTF-8");
